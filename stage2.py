...
class Ship():
    def __init__(self, x, y, name, max_hp, atk, type, gauge, max_gauge):
        ...
        self.frame_index = 0
        self.action = 0
        img = pygame.image.load(f'Phase3/img/{self.name}/Idel/{0}.png')
...
def dps():
    dps = Ship(0, 0, 'edps', 30, 15, 'dps', 0, 75)
    return dps
...
def main():
    ...
    time_scroll = 0
    # screen set up
    mothership = Ship(0, 0, 'Mothership', 300, 10, 'player', 0, 100)
    enemy1 = Ship(680, 180, 'etank', 50, 10, 'tank', 120, 200)
    enemy4 = Ship(800, 60, 'eMothership', 200, 10, 'player', 0, 100)
    tank1 = Ship(800, 560, 'tank', 50, 10, 'tank', 120, 200)
    enemylist[4] = enemy1
    enemylist[5] = enemy4
    mylist[3] = tank1
    d = None
    ...
    while run:
        ...
        if time_scroll >= 500:
            ...
            for spawn in range(3):
                if enemylist[spawn] == None:
                    etankadd(enemylist, spawn * 3)
            bootspeed = 0
            time += timespeed
            if tick == 1:
                etankadd(enemylist, 3)
                # etankadd(enemylist,0)
