...
def drawlose(screen):
    font = pygame.font.SysFont(None, 44)
    text = font.render('Defeated!', True, (255, 255, 255))
    screen.blit(text, (screen_width - 280, 10))
...
def draw_info(screen):
    ...
    atext = font.render('Buy and deploy tankship', True, (255, 255, 255))
    screen.blit(text, (screen_width - 280, 10))
    ...
    while run:
        ...
        if time_scroll >= 500:
            ...
            if inventory > 0:
                # print("ready to print")
                for event in pygame.event.get():
                    while event.type == pygame.MOUSEBUTTONDOWN & inventory > 0:
                        pos = pygame.mouse.get_pos()
                        if (pos[0] > 600 & pos[0] < 990):
                            if (pos[1] > 500 & pos[1] < 890):
                                i = int((pos[0] - 600) / 130) + int((pos[0] - 600) % 130 > 0) - 1
